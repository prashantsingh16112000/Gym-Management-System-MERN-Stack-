...
export function WorkoutProgramProvider({ children }) {
	...
	const categories = ["underweight", "overweight", "normalweight", "obese"]
	
	// Form Validation
	const schema = Joi.object({
		photoURL: Joi.string().uri().allow(""),
		name: Joi.string().min(3).max(20).message("Name should be between 4 and 20 characters"),
		...
		categories: Joi.array().items(Joi.string()) // 'categories' field as an array of strings
	});
	...
	const addWorkoutProgram = (values) => {
		const newWorkoutProgram = {
			...
			day: values.day,
			time: values.time,
			categories: values.categories // Include the categories field
